...
int main(){
    ...
    for(i = 0; i < MAX_LINES; i++){
        ...
        if(numCards[5]) hands[i].type = FIVE_OF_A_KIND;
        else if(numCards[4]) hands[i].type = FOUR_OF_A_KIND;
        else if(numCards[3]){
            if(numCards[2]) hands[i].type = FULL_HOUSE;
            else hands[i].type = THREE_OF_A_KIND;
        }
        else if(numCards[2] == 2) hands[i].type = TWO_PAIR;
        else if(!numCards[2]) hands[i].type = HIGH_CARD;
        else hands[i].type = ONE_PAIR;
        /*printf("%d/%d\n", hands[i].bid, hands[i].type);*/
    }
