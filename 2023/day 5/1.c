...
int main(){
    ...
    for(i = 7, j = 0; i < STR_SIZE && cInput[0][i] != '\0'; i += 1 + strToLongInt(cInput[0], i, &seeds[j++]));
    /*for(i = 0; i < numSeeds; i++) printf("%ld ", seeds[i]);
    printf("\n\n");*/

    for(i = 3, j = size = 0; i + size <= MAX_LINES;){
        /*printf("[%c, %d, %d, %d, %d, %c]", cInput[i][0], i, size, i+size<MAX_LINES, isDigit(cInput[i+size][0]), (cInput[i+size][0]));*/
    ...
    for(i = 0; i < numSeeds; i++){
        for(j = 0; j < 7; j++){
            /*printf("%ld -> ", seeds[i]);*/
            k = 0;
            while(k < convert[j].size && (seeds[i] >= convert[j].p[k][1] + convert[j].p[k][2] || seeds[i] < convert[j].p[k][1]))
                k++;
            if(k == convert[j].size) k--;
            seeds[i] = convertSeed(seeds[i], convert[j].p[k][0], convert[j].p[k][1], convert[j].p[k][2]);
            /*printf("%ld -> ", seeds[i]);*/
        }
        /*printf("\n");*/
    }
    ...
    for(i = 1; i < numSeeds; i++){
        /*printf("%ld / %ld / %d\n", seeds[i], res, seeds[i] < res);*/
        if(seeds[i] < res) res = seeds[i];
    }
